...
returns trigger as $$
        ...
        if not found then
          RAISE NOTICE 'item % not found for patient %', new.svpf_serv_code,new.svpf_patn__sequence;
        ...
        if ( TG_OP = 'UPDATE' ) then
           ...
           -- always recalculate the GST amount if the service amount has changed.
           if ( new.svpf_date_service > 'jul 1 2000' and old.svpf_amount != new.svpf_amount ) then
