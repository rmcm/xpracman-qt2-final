...
// ------------------------------------------------------------
void MReportViewer::setPrinterName( const QString & name )
{
    default_printer = name;
}

// ------------------------------------------------------------
void MReportViewer::setPrintProgram( const QString & program )
{
    default_printprogram = program;
}

// ------------------------------------------------------------
void MReportViewer::setPrinterSelectionOption( const QString & option )
{
    default_printeroption = option;
}

// ------------------------------------------------------------
void MReportViewer::setOutputFileName ( const QString & filename )
{
...
void MReportViewer::printReport(){
    ...
    printer->setFromTo(1, cnt);
        ...
        // use the default printer program
    if ( default_printprogram.length() )
        printer->setPrintProgram( default_printprogram );

        // use the default printer option
    if ( default_printeroption.length() )
        printer->setPrinterSelectionOption( default_printeroption );

        // set/reset an output file (null will reset it)
    printer->setOutputFileName( default_filename );
    
        // use the default printer
        // if the default printer is null, then setup the printer.
    if ( default_printer.length() )
        printer->setPrinterName( default_printer );
    else
        if ( default_filename.isEmpty() && !printer->setup(this) )
            return;
...
/** Print the report */
void MReportViewer::slotPrintReport(){
    printReport();
}
